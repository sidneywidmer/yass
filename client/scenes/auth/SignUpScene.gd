...
@onready var back_button_anon := %BackButtonAnon
@onready var error := %ErrorLabel

var signup_flow = null

func _ready() -> void:
	back_button.visible = !Player.is_anon() || !Player.is_authenticated()
	back_button_anon.visible = Player.is_anon() && Player.is_authenticated()
	...
	back_button_anon.pressed.connect(_on_back_button_anon_pressed)
...
func _on_back_button_anon_pressed():
...
func _on_signup_success(data):
	# If the player successfully signed up and is a authenticated anon user we link their account with the new ory one
	if Player.is_anon() && Player.is_authenticated():
		ApiClient.anon_link_account(data["session_token"], _on_signup_success_finish.bind(data), _on_signup_failed)
	else:
		_on_signup_success_finish(null, data)
	
func _on_signup_success_finish(link_data, ory_data):
	Player.set_anon_token("")
	Player.set_player(
		ory_data["session_token"],
		ory_data["session"]["identity"]["traits"]["email"],
		ory_data["session"]["identity"]["traits"]["name"]
	)
